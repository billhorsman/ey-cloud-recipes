...
if ['app_master', 'util', 'solo'].include?(node[:instance_role])
  ...
  execute "whenever" do
    ...
    command "bundle exec whenever --load-file #{release_path}/config/schedule.rb --set environment=#{rails_env} --update-crontab '#{appname}_#{node[:environment][:framework_env]}'"
    action :run 
  end
